...
void Menu::musicInit()
{
    ...
    this->m = libvlc_media_new_path(inst, "Assets/Menu/menu.mp3");
    this->_switchSound = libvlc_media_new_path(inst, "Assets/Menu/switch.mp3");
    this->_startSound = libvlc_media_new_path(inst, "Assets/Menu/start.mp3");
    this->mp = libvlc_media_player_new_from_media(m);
    this->_switchSoundp = libvlc_media_player_new_from_media(this->_switchSound);
    this->_startSoundp = libvlc_media_player_new_from_media(this->_startSound);
    libvlc_media_release(m);
    libvlc_media_release(this->_switchSound);
    libvlc_media_release(this->_startSound);
    libvlc_audio_set_volume(this->mp, 50);
    libvlc_audio_set_volume(this->_switchSoundp, 150);
    libvlc_audio_set_volume(this->_startSoundp, 80);
...
void Menu::musicClear()
{
    if (!this->_needToStop)
        return;
    libvlc_media_player_stop(mp);
    libvlc_media_player_stop(this->_switchSoundp);
    libvlc_media_player_stop(this->_startSoundp);
    libvlc_media_player_release(mp);
    libvlc_media_player_release(this->_switchSoundp);
    libvlc_media_player_release(this->_startSoundp);
    libvlc_release(inst);
...
void Menu::manageEnterButton()
{
    if (!this->_username.empty() && !this->usernametoolong) {
        if ((long unsigned int)this->_score > this->_lib.size() - 1)
            return;
        libvlc_media_player_play(this->_startSoundp);
        while (true) {
            if (libvlc_media_player_get_position(this->_startSoundp) >= 0.8)
                break;
        }
        if (this->_score == 0) {
...
void Menu::restartMusic(libvlc_media_player_t *mPlayer)
{
    libvlc_media_player_stop(mPlayer);
    libvlc_media_player_set_position(mPlayer, 0.0);
...
void Menu::applyEvent(const arcade::Key &key)
{
    ...
    if (key == arcade::Key::LEFT_ARROW && this->_score != 0) {
        restartMusic(this->_switchSoundp);
        this->_score--;
    }
...
void Menu::update(float deltaTime)
{
    ...
    if (libvlc_media_player_get_position(this->mp) >= 0.9)
        libvlc_media_player_set_position(this->mp, 0.0);
}
