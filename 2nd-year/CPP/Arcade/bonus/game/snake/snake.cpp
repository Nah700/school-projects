...
void Snake::initSound()
{
    ...
    this->fraise = libvlc_media_new_path(this->inst, "Assets/Snake/fraise.mp3");
    this->theme = libvlc_media_new_path(this->inst, "Assets/Snake/snake.mp3");
    this->nncbon = libvlc_media_new_path(this->inst, "Assets/Snake/nncbon.mp3");
    this->tropbon = libvlc_media_new_path(this->inst, "Assets/Snake/tropbon.mp3");
    this->dead = libvlc_media_new_path(this->inst, "Assets/Snake/death.mp3");
    this->fraisePlayer = libvlc_media_player_new_from_media(this->fraise);
    this->themePlayer = libvlc_media_player_new_from_media(this->theme);
    this->nncbonPlayer = libvlc_media_player_new_from_media(this->nncbon);
    this->tropbonPlayer = libvlc_media_player_new_from_media(this->tropbon);
    this->deadPlayer = libvlc_media_player_new_from_media(this->dead);
    libvlc_media_release(this->fraise);
    libvlc_media_release(this->theme);
    libvlc_media_release(this->nncbon);
    libvlc_media_release(this->tropbon);
    libvlc_media_release(this->dead);

    libvlc_audio_set_volume(this->fraisePlayer, 100);
    libvlc_audio_set_volume(this->themePlayer, 50);
    libvlc_audio_set_volume(this->nncbonPlayer, 100);
    libvlc_audio_set_volume(this->tropbonPlayer, 100);
    libvlc_audio_set_volume(this->deadPlayer, 100);
...
void Snake::musicClear()
{
    if (!this->_musicInit)
        return;
    sleep(1);
    libvlc_media_player_stop(this->themePlayer);
    libvlc_media_player_stop(this->fraisePlayer);
    libvlc_media_player_stop(this->nncbonPlayer);
    libvlc_media_player_stop(this->tropbonPlayer);
    libvlc_media_player_release(this->themePlayer);
    libvlc_media_player_release(this->fraisePlayer);
    libvlc_media_player_release(this->nncbonPlayer);
    libvlc_media_player_release(this->tropbonPlayer);
...
void Snake::applyEvent(const arcade::Key &key)
{
    if (key == arcade::Key::LEFT_ARROW) {
        ...
        if (this->isGameOver) {
            ...
            this->_score = 0;
            this->deathSound = 0;
            restartMusic(this->themePlayer);
        }
    }
}

void Snake::restartMusic(libvlc_media_player_t *player)
{
...
{
    if (type == 0) {
        restartMusic(this->nncbonPlayer);
        this->_speed = 0.005;
        this->_poweruptime = 0;
    } else if (type == 1) {
        restartMusic(this->tropbonPlayer);
        this->_speed = 0.3;
        this->_poweruptime = 0;
    } else if (type == 2) {
        restartMusic(this->tropbonPlayer);
        ...
        restartMusic(this->fraisePlayer);
        this->_score += 2;
...
void Snake::create_snake_pixel_wrapper()
{
    ...
    if (this->isGameOver) {
        if (this->deathSound == 1) {
            libvlc_media_player_stop(this->themePlayer);
            restartMusic(this->deadPlayer);
            while (true) {
                if (libvlc_media_player_get_position(this->deadPlayer) >= 0.8)
                    break;
            }
        }
        this->deathSound++;
...
void Snake::update(float deltaTime)
{
    ...
    if (this->clock - this->decale > this->_speed
        ...
        manageRainbowMod();
        if (libvlc_media_player_get_position(this->themePlayer) >= 0.9) {
            libvlc_media_player_set_position(this->themePlayer, 0.0);
        }
    }
